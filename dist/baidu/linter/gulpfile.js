...
const source = ['src/**/*.js', '!src/converter/**/*.*', '!src/linter/**/*.*', '!src/adaptor/**/*.*'];
...
gulp.task('test', () => {
  console.log('开始转换...');
  sequence('baidu', 'alibaba', 'toutiao')(() => {// console.log('转换完毕!');
  });
});
...
gulp.task('build', cb => {
  console.log('开始转换...');
  pump([gulp.src(source), uglifyES(UGLIFY_OPTIONS), gulp.dest('lib')], () => {
    console.log('转换完成！');
    cb && cb();
  });
});
